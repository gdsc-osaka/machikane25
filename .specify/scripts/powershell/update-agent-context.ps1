...
function Test-Environment {
    ...
    try {
        Get-Content -LiteralPath $NEW_PLAN -TotalCount 1 -ErrorAction Stop | Out-Null
    } catch {
        Write-Err "Plan file is not readable: $NEW_PLAN"
        exit 1
    }
...
function New-AgentFile {
    ...
    if ($escaped_lang -and $escaped_framework) {
        ...
        $techStackForTemplate = "- ($escaped_branch)"
    }
    ...
    if ($escaped_lang -and $escaped_framework) {
        ...
        $recentChangesForTemplate = "- ${escaped_branch}: Added"
    }
