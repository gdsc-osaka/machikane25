...
const aquariumServer = setupServer(
	http.post("https://aquarium.example.com/sync", async ({ request }: { request: Request }) => {
		const authorization = request.headers.get("authorization");
		expect(authorization).toBe("Bearer sync-token");
...
const originalAquariumEnv = {
	endpoint: process.env.AQUARIUM_SYNC_ENDPOINT,
	token: process.env.AQUARIUM_SYNC_TOKEN,
};

const restoreAquariumEnv = () => {
	if (typeof originalAquariumEnv.endpoint === "string") {
		process.env.AQUARIUM_SYNC_ENDPOINT = originalAquariumEnv.endpoint;
	} else {
		delete process.env.AQUARIUM_SYNC_ENDPOINT;
	}

	if (typeof originalAquariumEnv.token === "string") {
		process.env.AQUARIUM_SYNC_TOKEN = originalAquariumEnv.token;
	} else {
		delete process.env.AQUARIUM_SYNC_TOKEN;
	}
};
...
describe("Aquarium Sync Flow", () => {
	...
	beforeEach(() => {
		...
		process.env.AQUARIUM_SYNC_ENDPOINT =
			"https://aquarium.example.com/sync";
		process.env.AQUARIUM_SYNC_TOKEN = "sync-token";
		vi.resetModules();
	});
	...
	it(
		"captures failure via Sentry and retries successfully through admin action",
		async () => {
			const generatedPhoto = {
				boothId: "booth-42",
				photoId: "photo-777",
				imageUrl:
					"https://storage.example.com/photos/booth-42/photo-777.png",
				imagePath: "booths/booth-42/generatedPhotos/photo-777",
				createdAt: new Date("2025-01-01T10:00:00.000Z"),
			};

			const { sendToAquarium } = await import(
