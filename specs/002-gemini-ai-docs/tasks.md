...
- [X] T203 [P] [FOUND] Author failing converter tests `apps/photo/test/unit/infra/firestoreConverters.test.ts` validating Firestore 竊・domain mapping for all entities.
...
## Phase 3: User Story 1 窶・Visitor Generates AI Portrait (Priority: P1) 識 MVP
...
**Independent Test**: Firebase Emulator + msw scenario covering capture 竊・generate 竊・result render on kiosk device.

### Tests for User Story 1 笞・・
...
## Phase 4: User Story 2 窶・Visitor Retrieves Image Later (Priority: P2)
...
## Phase 5: User Story 3 窶・Staff Monitors Aquarium Sync (Priority: P3)
...
- [ ] T601 [P] [POLISH] Author Playwright E2E `apps/photo/test/e2e/photo.spec.ts` spanning booth capture 竊・token download 竊・admin retry, wired to Firebase Emulator.
...
- **Phase Dependencies**: Setup 竊・Foundational 竊・User Stories (US1 can start only after Foundational). US2 and US3 depend on US1 data contracts but can proceed in parallel once US1 API surfaces stabilise.
- **Within Stories**: Tests (笞・・ precede implementation; maintain SWR hooks/interfaces to avoid UI coupling. Ensure Storage/Sentry instrumentation tasks align with generated IDs before later phases.
